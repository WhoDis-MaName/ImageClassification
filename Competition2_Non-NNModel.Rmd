...
top_feature_names <- names(sorted_features)[1:300]
pca_data$y <- convoluted_train$y
...
```{r}
dtrain <- xgb.DMatrix(data = as.matrix(reduced_convoluted_train[, -301]), label = reduced_convoluted_train$y)
...
test_features <- convoluted_test[,intersect(names(reduced_convoluted_train), names(convoluted_test))]
