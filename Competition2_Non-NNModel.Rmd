...
summary(pca_result)
pca_data <- as.data.frame(pca_result$x[, 1:1000])
...
test_features <- convoluted_test[,names(features)]
pca_result_test <- prcomp(test_features, center = TRUE, scale. = TRUE)

pca_test <- as.data.frame(pca_result_test$x[, 1:1000])
